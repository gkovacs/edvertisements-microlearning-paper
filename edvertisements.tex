...
We have implemented microlearning tasks to fit 3 of the most common sizes -- 300x250, 728x90, and 200x90 -- which correspond to regular-sized, banner, and small ads.  If a microlearning task in the appropriate size is not available, we pick the nearest match and scale it to fit the available space. %Our system chooses the
...
Edvertisements are interactive microlearning tasks that we can show to users as they are surfing the web. We have built an browser extension that shows Edvertisements by inserting them into Facebook feeds, and by replacing web advertisements with them.
