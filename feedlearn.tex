...
It is implemented as a Chrome extension, as Facebook's API does not currently allow third-party developers to insert such interactive content into feeds.
In our preliminary user study, we compared Japanese vocabulary learning rates
among beginners when quizzes were inserted directly into feeds, as opposed to
inserting links reminding them to study, as is currently done by Facebook apps.
We find that on average, 
users answer more quizzes and learn more vocabulary
when the quizzes can be done without leaving their feeds.
...
FeedLearn is a Chrome extension which inserts interactive vocabulary quizzes into the user's Facebook feeds, as shown in \autoref{fig:feedlearn}. It supports several languages, but we will focus on the version that teaches basic Japanese vocabulary in this paper.
...
We recruited 14 users who had not previously studied Japanese but were interested in learning some basic vocabulary. They were voluntary participants recruited from online forums and Facebook groups related to Japanese culture. All of our participants were regular users of Facebook. %, spending at least 10 minutes on the site each day.
...
Injecting small, actionable tasks into users' Facebook feeds presents an opportunity to influence behavior. Although we have studied foreign-language vocabulary acquisition in this study, our approaches can potentially be extended to other flashcard-like educational content that can be interacted with directly inside a social news feed. One could also use a similar mechanism to encourage other behaviors that need to be done for short periods of time on a daily basis, such as small amounts of exercise.

Future work that remains to be done includes using an adaptive model to determine the optimal times to insert microlearning tasks into feeds. Another potential extension of this work is making the microlearning tasks more integrated with the Facebook environment to create a more social in-feed learning experience.
